...
@pytest.fixture
def mock_input():
    """Create a mock input that returns test data"""
    mock = Mock()
    mock.refresh.return_value = ["192.168.1.1", "10.0.0.5", "172.16.0.10"]
    mock.type = "edl"
    mock.url = "https://example.com/edl"
    return mock


@pytest.fixture
def mock_modifier():
    """Create a mock modifier that filters data"""
    mock = Mock()
    mock.refresh.return_value = ["192.168.1.1", "10.0.0.5"]  # Filters out one IP
    mock.type = "ipv4"
    return mock
...
@pytest.fixture
def mock_output():
    """Create a mock output that formats data"""
    mock = Mock()
    mock.refresh.return_value = "192.168.1.1\n10.0.0.5"
    mock.type = "ipv4"
    return mock
...
@pytest.fixture
def sample_new_pipeline(mock_input, mock_modifier, mock_output, sample_refresh_rate):
    """Create a sample NewPipeline with valid data"""
    return NewPipeline(
        group="test_group",
        name="Test Pipeline",
...
@pytest.fixture
def sample_pipeline(sample_new_pipeline):
    """Create a sample Pipeline with all required fields"""
    ...
    return Pipeline(
        ...
        **sample_new_pipeline.model_dump()
    )
...
class TestNewPipeline:
    ...
    def test_new_pipeline_creation(self, sample_new_pipeline):
        """Test creating a NewPipeline with all fields"""
        assert sample_new_pipeline.group == "test_group"
        assert sample_new_pipeline.name == "Test Pipeline"
        assert sample_new_pipeline.description == "A test pipeline for unit testing"
        assert sample_new_pipeline.state == State.ENABLED
        assert len(sample_new_pipeline.inputs) == 1
        assert len(sample_new_pipeline.modifiers) == 1
        assert len(sample_new_pipeline.outputs) == 1

    def test_new_pipeline_state_defaults_to_enabled(self, mock_input, sample_refresh_rate):
    ...
    def test_new_pipeline_modifiers_default_empty(self, mock_input, sample_refresh_rate):
    ...
    def test_new_pipeline_outputs_default_to_all(self, mock_input, sample_refresh_rate):
    ...
    def test_new_pipeline_description_optional(self, mock_input, sample_refresh_rate):
    ...
    def test_new_pipeline_requires_group(self, mock_input, sample_refresh_rate):
    ...
    def test_new_pipeline_requires_name(self, mock_input, sample_refresh_rate):
    ...
    def test_new_pipeline_multiple_inputs(self, sample_refresh_rate):
        """Test pipeline with multiple inputs"""
        mock_input1 = Mock()
        mock_input1.type = "edl"
        mock_input2 = Mock()
        mock_input2.type = "edl"
        mock_input3 = Mock()
        mock_input3.type = "edl"
